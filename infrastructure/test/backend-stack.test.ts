...
describe('BackendStack', () => {
  ...
  describe('RAG Ingestion Lambda', () => {
    it('does NOT live in BackendStack (hoisted to PlatformStack in Phase 4)', () => {
      // Phase 4 moved RAG ingestion to PlatformStack alongside its
      // bucket and table. BackendStack has zero Lambda functions
      // after Phases 3 + 4.
      template.resourceCountIs('AWS::Lambda::Function', 0);
    });

    it('does NOT create the S3 bucket notification (moved to PlatformStack)', () => {
      template.resourceCountIs('Custom::S3BucketNotifications', 0);
    });

    it('does NOT publish ingestion function-name SSM (moved to PlatformStack)', () => {
      const params = template.findResources('AWS::SSM::Parameter', {
        Properties: { Name: '/test-project/rag/ingestion-function-name' },
      });
  ...
  describe('SSM parameters', () => {
    ...
    it('does NOT publish RAG ingestion Lambda ARN (moved to PlatformStack)', () => {
      const params = template.findResources('AWS::SSM::Parameter', {
        Properties: { Name: '/test-project/rag/ingestion-lambda-arn' },
      });
      expect(Object.keys(params).length).toBe(0);
    });
  });
