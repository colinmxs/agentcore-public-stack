...
This document provides a comprehensive reference for all GitHub Variables and Secrets used in the AgentCore Public Stack deployment workflows. The table below shows all configuration values across the five deployment workflows: Infrastructure, App API, Inference API, Frontend, and Gateway.
...
| CDK_APP_API_ENTRA_REDIRECT_URI | Variable | No | None | App API | Microsoft Entra ID OAuth redirect URI for authentication callback |
...
| CDK_INFERENCE_API_ENABLE_GPU | Variable | No | `false` | Inference API | Enable GPU support for Inference API runtime |
...
| ENV_INFERENCE_API_OAUTH_CALLBACK_URL | Variable | No | None | Inference API | OAuth callback URL for external MCP tool authentication |
| ENV_INFERENCE_API_OUTPUT_DIR | Variable | No | `output` | Inference API | Directory path for output files in runtime container |
| ENV_INFERENCE_API_TAVILY_API_KEY | Secret | No | None | Inference API | Tavily API key for web search integration |
| ENV_INFERENCE_API_UPLOAD_DIR | Variable | No | `uploads` | Inference API | Directory path for uploaded files in runtime container |
