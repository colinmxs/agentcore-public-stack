...
import { ToolUseContentBlock } from '../../../../services/models/message.model';
@Component({
...
export class ToolUseComponent {
  /** The tool use data to display */
  toolUse = input.required<ToolUseContentBlock>();
  ...
  inputKeysPreview = computed(() => {
    const keys = Object.keys(this.toolUse().input);
    if (keys.length === 0) {
      return '{ }';
    }
    return `{ ${keys.join(', ')} }`;
  ...
  formattedJson = computed(() => {
    return JSON.stringify(this.toolUse().input, null, 2);
  });
