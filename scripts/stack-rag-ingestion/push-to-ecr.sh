...
# Script: Push Docker Image to ECR for RAG Ingestion Lambda
...
ensure_ecr_repo() {
    ...
    if [ ${exit_code} -ne 0 ]; then
        ...
        aws ecr create-repository \
            ...
            --tags Key=Project,Value=${CDK_PROJECT_PREFIX} Key=ManagedBy,Value=GitHubActions Key=Stack,Value=RagIngestion
...
main() {
    log_info "Pushing RAG Ingestion Lambda Docker image to ECR..."
    ...
    SSM_PARAM_NAME="/${CDK_PROJECT_PREFIX}/rag-ingestion/image-tag"
    ...
    aws ssm put-parameter \
        ...
        --value "${IMAGE_TAG}" \
        --type "String" \
        --description "Current image tag for RAG Ingestion Lambda deployed to ECR" \
        --overwrite \
